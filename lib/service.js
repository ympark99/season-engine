...
import { DEFAULT_PARAMS, DEFAULT_CAL, analyze } from './engine.js';

export const FETCH_DAYS = 2 * 365 + 150;   // 표시 2년 + 판정 워밍업(60일 이평·52주 고점)
export const KEEP = 505;                    // 화면에 보내는 최근 봉 수 ≈ 2년
...
export async function engineState() {
  ...
  return { params: { ...DEFAULT_PARAMS, ...(e?.params || {}) }, cal: e?.cal || DEFAULT_CAL, trainedAt: e?.trainedAt || null, fit: e?.fit || null, anchors: e?.anchors || [] };
}
...
export async function refresh(item, { full = false } = {}) {
  ...
  if (old?.d?.length) {
    ...
    if (r.d.some((d, i) => idx.has(d) && Math.abs(r.c[i] / idx.get(d) - 1) > 0.02)) { old = null; got = await fetchRange(item, startDate()); excd = got.excd || excd; }
    else got = r;
  } else { got = await fetchRange(item, startDate()); excd = got.excd || excd; }
  const map = new Map();
  if (old) old.d.forEach((d, i) => map.set(d, [old.c[i], old.v[i]]));
  got.d.forEach((d, i) => map.set(d, [got.c[i], got.v[i]]));
  const d = [...map.keys()].sort();
  const bars = trim({ d, c: d.map(k => map.get(k)[0]), v: d.map(k => map.get(k)[1]) });
